...
socklen_t const Just1RCe::TcpSocket::kInetSocketAddrLen = sizeof(InetSocketAddress);

TcpSocket::TcpSocket(std::string const &port_number)
try : cur_inet_sock_addr_len_(kInetSocketAddrLen), socket_fd_(socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)),
    is_listen_only_(true)
{
    ...
    if (setsockopt(socket_fd_, SOL_SOCKET, SO_REUSEADDR, &sock_reuse_opt, sizeof(sock_reuse_opt)) == -1)
        throw std::runtime_error(JUST1RCE_SRCS_SOCKET_SET_OPTION_ERROR);

    if (bind(socket_fd_, reinterpret_cast<struct sockaddr *>(&inet_sock_address_), kInetSocketAddrLen) == -1)
    ...
    if (flag == -1)
        throw std::runtime_error(JUST1RCE_SRCS_FCNTL_GETFL_ERROR);
...
try : cur_inet_sock_addr_len_(kInetSocketAddrLen),
    socket_fd_(accept(listen_sock_fd, reinterpret_cast<struct sockaddr *>(&inet_sock_address_),
...
int TcpSocket::socket_fd()
{
    return socket_fd_;
}

bool TcpSocket::is_listen_only()
{
    return is_listen_only_;
}

// return port number in host order
in_port_t TcpSocket::GetPortNum()
{
